...
# Read index.html if template or construct cleanly
...
Clique em "Gerar Gancho" para obter um script otimizado para o seu perfil!
    ...
    <script>
        ...
        function renderTOC(filteredList) {
            if (!filteredList) filteredList = EBOOK_CHAPTERS;
            ...
            container.innerHTML = filteredList.map(function(ch) {
                const originalIdx = EBOOK_CHAPTERS.findIndex(function(c) { return c.id === ch.id; });
        ...
        function handleSearch(query) {
            ...
            if (filtered.length > 0 && searchQuery.length > 1) {
                const firstMatchIdx = EBOOK_CHAPTERS.findIndex(function(c) { return c.id === filtered[0].id; });
        ...
        function filterChapters(query) {
            ...
            return EBOOK_CHAPTERS.filter(function(ch) { 
                return ch.title.toLowerCase().indexOf(q) !== -1 || 
                       ch.module.toLowerCase().indexOf(q) !== -1 || 
                       ch.content.toLowerCase().indexOf(q) !== -1;
            });
        }
...
print("Regenerated index.html safely with raw template!")
